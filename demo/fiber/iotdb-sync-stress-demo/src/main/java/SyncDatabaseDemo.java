    ...
    public static String execQuery(String sql) throws InterruptedException, ExecutionException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String temperature = rs.getString("root.zm.wf01.wt01.temperature");
                boolean status = rs.getBoolean("root.zm.wf01.wt01.status");
                queryResult.append("time: ").append(time.toString()).append(" temperature:").append(temperature).append(" status: ").append(status).append("\n");
            }
    ...
    public static void testAsyncQuery() throws Exception {
        ...
        for (int i = 0; i < requestCount; i++) {
            ...
            CompletableFuture<String> cf = CompletableFuture.supplyAsync(() -> {
                ...
                try {
                    startSignal.await();
                    result = execQuery("select * from root.zm");
                } catch (Exception e) {
                }
    ...
    public static void testSyncQuery() throws Exception {
        ...
        Runnable r = new Runnable() {
            ...
            public void run() {
                try {
                    startSignal.await();
                    String sql = "select * from root.zm";
                    String result;
