...
#if INCLUDE_CDS
  if (UseAppCDS && UseSharedSpaces) {
    warning("Sharing is only supported for boot loader classes because bootstrap classpath has been appended");
    FileMapInfo::current_info()->header()->set_has_ext_or_app_classes(false);
  }
#endif
...
void ClassLoaderExt::setup_app_search_path() {
  ...
  _app_class_paths_start_index = ClassLoader::num_classpath_entries();
...
void ClassLoaderExt::process_jar_manifest(ClassPathEntry* entry,
  ...
  if (cp_attr != NULL && strlen(cp_attr) > 0) {
    trace_class_path(tty, "found Class-Path: ", cp_attr);
