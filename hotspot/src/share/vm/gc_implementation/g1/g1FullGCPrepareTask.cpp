...
G1FullGCPrepareTask::G1CalculatePointersClosure::G1CalculatePointersClosure(CMBitMap* bitmap,
...
void G1FullGCPrepareTask::G1CalculatePointersClosure::free_humongous_region(HeapRegion* hr) {
  ...
  _g1h->free_humongous_region(hr, &dummy_free_list, true);
  prepare_for_compaction(hr);
...
void G1FullGCPrepareTask::prepare_serial_compaction() {
  ...
  for (GrowableArrayIterator<HeapRegion*> it = cp->regions()->begin(); it != cp->regions()->end(); ++it) {
    ...
    if (!cp->is_initialized()) {
      ...
      assert(!current->isHumongous(), "Should be no humongous regions in compaction queue");
...
bool G1FullGCPrepareTask::G1CalculatePointersClosure::freed_regions() {
  if (_humongous_regions_removed.length() > 0) {
