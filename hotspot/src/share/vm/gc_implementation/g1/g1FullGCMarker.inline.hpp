...
#ifndef SHARE_VM_GC_IMPLEMENTATION_G1_G1MARKSTACK_INLINE_HPP
#define SHARE_VM_GC_IMPLEMENTATION_G1_G1MARKSTACK_INLINE_HPP
...
template <class T> inline void G1FullGCMarker::mark_and_push(T* p) {
  ...
  if (!oopDesc::is_null(heap_oop)) {
    ...
    if (mark_object(obj)) {
      _oop_stack.push(obj);
      assert(_bitmap->isMarked(obj), "Must be marked now - map self");
    } else {
      assert(_bitmap->isMarked(obj),
             "Must be marked by other");
    }
  }
}
...
inline void G1FullGCMarker::follow_object(oop obj) {
  assert(_bitmap->isMarked(obj), "should be marked");
  if (obj->is_objArray()) {
    ...
    if (VerifyDuringGC) {
      if (obj->is_instance() && obj->klass()->oop_is_instanceRef()) {
        return;
      }
...
void G1FullGCMarker::drain_stack() {
  do {
    ...
    while (pop_object(obj)) {
      assert(_bitmap->isMarked(obj), "must be marked");
      follow_object(obj);
    }
...
#endif // SHARE_VM_GC_IMPLEMENTATION_G1_G1MARKSTACK_INLINE_HPP
