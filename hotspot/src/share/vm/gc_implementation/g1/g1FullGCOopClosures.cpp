...
void G1FullKeepAliveClosure::do_oop(oop* p) { do_oop_nv(p); }
void G1FullKeepAliveClosure::do_oop(narrowOop* p) { do_oop_nv(p); }
...
void G1MarkAndPushClosure::do_class_loader_data(ClassLoaderData* cld) {
  _marker->follow_cld(cld);
}

bool G1IsAliveClosure::do_object_b(oop p) {
  return  _bitmap->isMarked(p);
}
