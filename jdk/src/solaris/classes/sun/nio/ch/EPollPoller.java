...
class EPollPoller extends Poller {
    ...
    void implRegister(int fdVal) throws IOException {
        // re-arm
        int err = EPoll.epollCtl(epfd, EPOLL_CTL_MOD, fdVal, (event | EPOLLONESHOT));
        if (err == ENOENT)
            err = EPoll.epollCtl(epfd, EPOLL_CTL_ADD, fdVal, (event | EPOLLONESHOT));
        if (err != 0)
    ...
    void implDeregister(int fdVal) {
        EPoll.epollCtl(epfd, EPOLL_CTL_DEL, fdVal, 0);
    }
    ...
    int poll(int timeout) throws IOException {
        ...
        int n = EPoll.epollWait(epfd, address, MAX_EVENTS_TO_POLL);
        int i = 0;
        while (i < n) {
