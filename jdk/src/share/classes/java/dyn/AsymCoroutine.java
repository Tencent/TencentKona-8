...
public class AsymCoroutine<InT, OutT> extends CoroutineBase implements Iterable<OutT> {
	...
	public AsymCoroutine(String name) {
		super(name);
		target = null;
		threadSupport.addCoroutine(name ,this, -1);
	}

	public AsymCoroutine(String name,long stacksize) {
		super(name);
		target = null;
		threadSupport.addCoroutine(name ,this, stacksize);
	}

	public AsymCoroutine(String name,AsymRunnable<? super InT, ? extends OutT> target) {
		super(name);
		this.target = target;
		threadSupport.addCoroutine(name ,this, -1);
	}

	public AsymCoroutine(String name,AsymRunnable<? super InT, ? extends OutT> target, long stacksize) {
		super(name);
		this.target = target;
		threadSupport.addCoroutine(name ,this, stacksize);
	}
