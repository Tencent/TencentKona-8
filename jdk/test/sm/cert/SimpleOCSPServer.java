...
import static sun.security.provider.certpath.OCSPResponse.ResponseStatus;
...
public class SimpleOCSPServer {
    ...
    public SimpleOCSPServer(InetAddress addr, int port, KeyStore ks,
        ...
        sigAlgId = AlgorithmId.get(getDefaultSigAlgForKey(signerKey));
        ...
        switch (kAlg.toUpperCase(Locale.ENGLISH)) {
            ...
            case "RSA":
                return ifcFfcStrength(KeyUtil.getKeySize(k))
                    + "with" + kAlg;
            case "EC":
                return isSM2Key((ECPrivateKey) k)
                        ? "SM3withSM2"
                        : ecStrength(KeyUtil.getKeySize(k)) + "withECDSA";
            case "RSASSA-PSS":
            case "ED25519":
            case "ED448":
                return kAlg;
    ...
    // Check if SM2 private key
    private static boolean isSM2Key(ECPrivateKey k) {
        return SM2ParameterSpec.ORDER.equals(k.getParams().getOrder());
    }
    ...
    private class OcspHandler implements Runnable {
        ...
        public void run() {
            ...
            try (Socket ocspSocket = sock;
                ...
                OCSPResponse.ResponseStatus respStat = OCSPResponse.ResponseStatus.INTERNAL_ERROR;
                try {
    ...
    public class LocalOcspResponse {
        ...
        private byte[] getBytes() throws IOException {
            ...
            if (responseStatus == OCSPResponse.ResponseStatus.SUCCESSFUL &&
                    respItemMap != null) {
