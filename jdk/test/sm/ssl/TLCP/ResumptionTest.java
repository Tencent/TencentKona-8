...
/*
 * @test
 * @summary Session resumption on TLCP 1.1.
 * @compile ../../Utils.java
...
//        System.setProperty("javax.net.debug", "all");
     ...
     */
    protected final CountDownLatch firstClientCondition = new CountDownLatch(1);
    protected final CountDownLatch secondClientCondition = new CountDownLatch(1);
    ...
    protected void doServerSide() throws Exception {
        ...
        // Try to accept the first connection in 30 seconds.
        SSLSocket firstSSLSocket;
        try {
            sslServerSocket.setSoTimeout(300000);
            firstSSLSocket = (SSLSocket)sslServerSocket.accept();
        ...
        try {
            ...
            boolean clientIsReady =
                    firstClientCondition.await(30L, TimeUnit.SECONDS);

            if (clientIsReady) {
                // Run the application in server side.
                runServerApplication(firstSSLSocket);
            ...
            firstSSLSocket.close();
        }

        // Try to accept the second connection in 30 seconds.
        SSLSocket secondSSLSocket;
        try {
            secondSSLSocket = (SSLSocket)sslServerSocket.accept();
        } catch (SocketTimeoutException ste) {
            // Ignore the test case if no connection within 30 seconds.
            System.out.println(
                    "No incoming client connection in 30 seconds. " +
        ...
        try {
            ...
            // to the expected client.
            boolean clientIsReady =
                    secondClientCondition.await(30L, TimeUnit.SECONDS);

            if (clientIsReady) {
                // Run the application in server side.
                runServerApplication(secondSSLSocket);
            } else {    // Otherwise, ignore
                // We don't actually care about plain socket connections
                // for TLS communication testing generally.  Just ignore
                // the test if the accepted connection is not linked to
    ...
    protected void doClientSide() throws Exception {
        ...
        try (SSLSocket sslSocket = (SSLSocket)sslsf.createSocket()) {
            ...
            firstClientCondition.countDown();
            ...
            firstCreationTime = sslSocket.getSession().getCreationTime();
        }

        long secondCreationTime;
        try (SSLSocket sslSocket = (SSLSocket)sslsf.createSocket()) {
            try {
                configureClientSocket(sslSocket);
                InetAddress serverAddress = this.serverAddress;
            ...
            // Signal the server, the client is ready to communicate.
            secondClientCondition.countDown();
            ...
            // Run the application in client side.
            runClientApplication(sslSocket);
            secondCreationTime = sslSocket.getSession().getCreationTime();
        }

        if (firstCreationTime == -1L || firstCreationTime != secondCreationTime) {
            throw new RuntimeException("Incorrect session creation time: "
                    + "firstCreationTime=" + firstCreationTime
                    + ", secondCreationTime=" + secondCreationTime);
        } else {
