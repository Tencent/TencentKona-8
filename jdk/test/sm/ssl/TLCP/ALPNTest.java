...
/*
 * @test
 * @summary TLCP 1.1 with Application Layer Protocol Negotiation.
 * @compile ../../Utils.java
 * @run main/othervm -Djdk.tls.client.protocols=TLCPv1.1
 *                   ALPNTest h2 HTTP/1.1,h2 h2
 * @run main/othervm -Djdk.tls.client.protocols=TLCPv1.1
 *                   ALPNTest HTTP/1.1,h2 h2,HTTP/1.1 HTTP/1.1
 * @run main/othervm -Djdk.tls.client.protocols=TLCPv1.1
 *                   ALPNTest h2 HTTP/1.1 none
 */
...
//        System.setProperty("javax.net.debug", "all");
        ...
        String[] serverAppProtocols = args[0].split(",");
        String[] clientAppProtocols = args[1].split(",");
        String expectedNegotiated = args[2];
        new ALPNTest(serverAppProtocols, clientAppProtocols, expectedNegotiated).run();
    }

    private final String[] serverAppProtocols;
    private final String[] clientAppProtocols;
    private final String expectedNegotiated;

    private final boolean expectedFail;

    public ALPNTest(String[] serverAppProtocols, String[] clientAppProtocols,
                    String expectedNegotiated) {
        this.serverAppProtocols = serverAppProtocols;
        this.clientAppProtocols = clientAppProtocols;
        this.expectedNegotiated = expectedNegotiated;

        expectedFail = "none".equals(expectedNegotiated);
    }
    ...
    protected void runServerApplication(SSLSocket socket) throws Exception {
        ...
        try {
            ...
            if (expectedFail) {
                throw new RuntimeException("Expected SSLHandshakeException wasn't thrown");
            }
        } catch (SSLHandshakeException se) {
            if (expectedFail) {
                System.out.println("Expected SSLHandshakeException: " + se.getMessage());
            } else {
                throw se;
            }
        }
    ...
    protected void configureClientSocket(SSLSocket socket) {
        ...
        params.setApplicationProtocols(clientAppProtocols);
    ...
    protected void configureServerSocket(SSLServerSocket socket) {
        ...
        SSLParameters params = socket.getSSLParameters();
        params.setApplicationProtocols(serverAppProtocols);
        socket.setSSLParameters(params);
    }
    ...
    protected void doClientSide() throws Exception {
        ...
        try (SSLSocket sslSocket = (SSLSocket)sslsf.createSocket()) {
            ...
            appProtocol = appProtocol == null ? "none" : appProtocol;
            if (!expectedNegotiated.equals(appProtocol)) {
                throw new RuntimeException("Unexpected negotiated protocol: "
                        + appProtocol);
            } else {
                System.out.println("Negotiated protocol: " + appProtocol);
            }
        }
    }
