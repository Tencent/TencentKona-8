...
public class CoroutineTest {
	...
	public void asymSequence() {
		AsymCoroutine<Void, Void> coro = new AsymCoroutine<Void, Void>("coro_asymSequence") {
	...
	public void asymMultiSequence() {
		...
		for (int j = 4; j >= 0; j--) {
			...
			final int i = j;
			coro = new AsymCoroutine<Void, Void>("coro_asymMultiSequence") {
				protected Void run(Void value) {
					seq.append("b" + i);
					if (last != null)
						last.call();
	...
	public void asymReturnValue() {
		AsymCoroutine<Integer, Integer> coro = new AsymCoroutine<Integer, Integer>("coro_asymReturnValue") {
